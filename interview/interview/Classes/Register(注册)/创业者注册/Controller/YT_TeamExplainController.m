...
@interface YT_TeamExplainController ()<UITextViewDelegate>
@property (nonatomic,weak)UITextView *explainTextView;
@property (nonatomic,weak)UILabel *wordLable;
@end

@implementation YT_TeamExplainController
...
- (void)createUI
{
    ...
    self.explainTextView.text = [YT_CreatePersonInfoModel sharedPersonInfoModel].apntTeamIntro;
...
- (void)sendRequest
{
    ...
    [YBHttpNetWorkTool post:Url_ItemProduct_Info params:params success:^(id json) {
        ...
        if (statusCode == 1) {
            ...
            [YT_CreatePersonInfoModel sharedPersonInfoModel].apntTeamIntro = self.explainTextView.text;
            [self.navigationController popViewControllerAnimated:YES];
