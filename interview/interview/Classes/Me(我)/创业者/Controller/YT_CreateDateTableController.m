...
@property (nonatomic,strong)YBDotView *dotGreen;
@end
...
//    self.dotGreen = [[YBDotView alloc] init];
//    [self.commentDateBtn addSubview:self.dotGreen];
...
- (void)sendDataTableRequest:(NSDictionary *)params andIsNewData:(BOOL)isNewData
{
    ...
    [YBHttpNetWorkTool post:Url_Date_list params:params success:^(id json) {
        ...
        if (status == 1) {
            ...
            if (isNewData) {//最新数据
                if (statusCreate == 1) {
                    if (weakSelf.page == totalPage || weakSelf.dataArrayWait.count == 0) {
                    ...
                    if (weakSelf.page == totalPage || weakSelf.dataArrayComment.count == 0) {
                    ...
                    if (weakSelf.page == totalPage || weakSelf.dataArrayFinish.count == 0) {
