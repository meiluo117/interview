...
@interface YT_investorDate3Controller ()
...
@implementation YT_investorDate3Controller
...
- (void)createData
{
    ...
    } header:[YT_InvestorPersonInfoModel sharedPersonInfoModel].ticket ShowWithStatusText:@""];
}
...
- (void)createUI
{
    ...
    self.NavTitle = @"待约谈";
    self.sureBtn.backgroundColor = titleColor;
...
- (IBAction)sureBtnClick:(UIButton *)sender {
    YT_WS(weakSelf);
    NSDictionary *params = @{@"type":@"1",@"orderId":[YT_DateInvestorOrderRequestModel sharedModel].data.orderId};
    [YBHttpNetWorkTool post:Url_Date_InvestorSureMeet params:params success:^(id json) {
        
        NSInteger status = [json[@"code"] integerValue];
        if (status == 1) {
            
            [SVProgressHUD showSuccessWithStatus:json[@"msg"]];
            YT_investorDate4Controller *investorDate4Vc = [[YT_investorDate4Controller alloc] init];
            investorDate4Vc.orderID = weakSelf.orderID;
            [weakSelf.navigationController pushViewController:investorDate4Vc animated:YES];
        }else{
