...
@interface YT_investorDate4Controller ()
...
@property (weak,nonatomic) UILabel *noComment;
...
@implementation YT_investorDate4Controller
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    self.scrollV.contentSize = CGSizeMake(ScreenWidth, CGRectGetMaxY(self.noComment.frame) + 50);
}
...
- (void)createUI
{
    ...
    //暂无评论
    UILabel *noComment = [[UILabel alloc] initWithFrame:CGRectMake((ScreenWidth - 100) / 2, CGRectGetMaxY(comment.frame) + 40, 100, 20)];
    noComment.textColor = YT_Color(221, 221, 221, 1);
    noComment.textAlignment = NSTextAlignmentCenter;
    noComment.font = [UIFont systemFontOfSize:14];
    noComment.text = @"暂无评论";
    self.noComment = noComment;
    [self.scrollV  addSubview:self.noComment];
}
